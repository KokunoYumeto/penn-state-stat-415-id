#!/usr/bin/env python3
"""Credential-free static-HTTPS readback for the exact C4 content commit.
...
RECEIPT = ROOT / "00_control" / "GITHUB_CONTENT_READBACK_2026-08-29_C4_COMMIT.json"
REPOSITORY = "KokunoYumeto/penn-state-stat-415-id"
PARENT = "9eaeb28cdf1e9e25117ab0ee68535e5bb2c66158"
COMMIT = "9b10b3e04b451232b1233d0b35cf31c3860d63db"
EXPECTED_FILES = 62
SCHEMA = "o006.c140.c4.github-content-readback.v1"
...
def download(url: str) -> bytes:
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "o006-c140-c4-static-readback/1.0",
        },
        method="GET",
    )
...
def build_receipt() -> dict[str, object]:
    ...
    if resolved_commit != COMMIT or resolved_parent != PARENT:
        raise RuntimeError("local commit ancestry differs from the fixed C4 boundary")
...
def main() -> int:
    ...
    if args.write:
        ...
        print(
            f"C4 commit readback written: {receipt['file_count']} files / "
    ...
    try:
        ...
        raise RuntimeError("C4 commit readback receipt is unavailable") from exc
    if actual != expected:
        raise RuntimeError("C4 commit readback receipt is stale or non-deterministic")
    print(
        f"C4 commit readback check passed: {receipt['file_count']} files / "
