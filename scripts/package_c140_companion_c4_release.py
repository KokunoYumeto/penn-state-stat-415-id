#!/usr/bin/env python3
"""Package the cumulative C140 original-companion C4 checkpoint.

The local contract preserves the anonymously verified 49-file C3
publication byte for byte and appends eight compact C4 artifacts.  It performs
...
BASE_PACKAGE_RECEIPT = (
    ROOT / "build" / "C140_COMPANION_C3_RELEASE_PACKAGE_RECEIPT.json"
)
BASE_PUBLIC_READBACK = (
    ROOT
    / "00_control"
    / "ZENODO_PUBLIC_READBACK_2026-08-29_C140_COMPANION_C3.json"
)
PACKAGE_RECEIPT = ROOT / "build" / "C140_COMPANION_C4_RELEASE_PACKAGE_RECEIPT.json"

OFFLINE_NAME = "05_C140_COMPANION_C4_OFFLINE_READER.zip"
SOURCE_NAME = "15_C140_COMPANION_C4_SOURCE_BACKEND.zip"
NOTES_NAME = "25_C140_COMPANION_C4_RELEASE_NOTES.md"
LICENSE_NAME = "35_C140_COMPANION_C4_LICENSE.md"
QA_NAME = "45_C140_COMPANION_C4_STATIC_QA_EVIDENCE.zip"
MANIFEST_NAME = "96_C140_COMPANION_C4_FULL_UNION_MANIFEST.csv"
CHECKSUM_NAME = "SHA256SUMS_C140_COMPANION_C4.txt"
ROOT_NAME = "97_C140_COMPANION_C4_FULL_UNION_ROOT_RECEIPT.json"

VERSION = "2026.08.29.c140-companion-c4"
SCHEMA = "o006.c140.companion-c4-release-package.v1"
BASE_SCHEMA = "o006.c140.companion-c3-release-package.v1"
BASE_READBACK_SCHEMA = "o006.c140.zenodo-c140-companion-c3-publication.v1"
...
EXPECTED_HTML_FILES = 64
EXPECTED_HTML_BYTES = 3_024_784
EXPECTED_BACKEND_FILES = 4
EXPECTED_BACKEND_BYTES = 359_500
EXPECTED_BACKEND_ENTITIES = 1_113
EXPECTED_BACKEND_RELATIONS = 1_424
EXPECTED_SOURCE_BYTES = 683_211
EXPECTED_PROBLEMS = 114
    ...
    inventory_payload = canonical_json(
        {
            ...
            "schema": "o006.c140.companion-c4-archive-inventory.v1",
            "status": "pass",
...
def validate_base_public_union() -> tuple[
    ...
    readback_payload = read_identity(
        ...
        BASE_PUBLIC_READBACK_SHA256,
        "C3 anonymous public readback",
    )
    receipt = json_object(receipt_payload, "C3 package receipt")
    readback = json_object(readback_payload, "C3 public readback")
    ...
    if (
        ...
        raise RuntimeError("C3 package contract differs")
    ...
    if (
        ...
        raise RuntimeError("C3 public readback contract differs")
...
def validate_c4_boundary() -> tuple[
    dict[str, Any], dict[str, Any], dict[str, dict[str, object]]
]:
    receipt_identities = validate_pinned_receipts()
    build = json_object(
        (COMPONENT / "build" / "C4_BUILD_RECEIPT.json").read_bytes(),
        "C4 build receipt",
    )
    qa = json_object(
        (COMPONENT / "build" / "C4_QA_RECEIPT.json").read_bytes(),
        "C4 QA receipt",
    )
    ...
    if (
        ...
        raise RuntimeError("C4 cumulative build contract differs")
    ...
    if {
        ...
        raise RuntimeError("C4 build simulation-receipt closure differs")

    seen_sources: set[str] = set()
    for row in source_rows:
        if not isinstance(row, dict):
            raise RuntimeError("C4 source row is malformed")
        relative = str(row.get("path", ""))
        shared.validate_relative(relative)
        if relative in seen_sources:
            raise RuntimeError(f"duplicate C4 source row: {relative}")
        seen_sources.add(relative)
        path = COMPONENT / relative
        if path.is_symlink() or not path.is_file():
            raise RuntimeError(f"missing C4 source: {relative}")
        payload = path.read_bytes()
        if len(payload) != row.get("bytes") or sha256(payload) != row.get("sha256"):
            raise RuntimeError(f"C4 source identity differs: {relative}")
    if sum(int(row["bytes"]) for row in source_rows) != EXPECTED_SOURCE_BYTES:
        raise RuntimeError("C4 source-byte census differs")

    if (
        qa.get("schema") != "o006.c140.companion-cumulative-c4-qa.v1"
        ...
        != PINNED_RECEIPTS["build/C4_BUILD_RECEIPT.json"][1]
        ...
        raise RuntimeError("C4 cumulative QA contract differs")
...
def release_notes(build: dict[str, Any], qa: dict[str, Any]) -> bytes:
    ...
    return (
        "# C140 original companion — C4 checkpoint\n\n"
        "Status: **partial but coherent**. The complete Penn State STAT 415 "
        "Indonesian spine, exact Random completeness donor, and all 49 "
        "anonymously verified C3 files remain byte-identical. This version adds "
        "the complete bounded C4 independent-study mastery batch.\n\n"
        "C4 adds MS00–MS06: probability/distribution review, order statistics, "
        "estimation and bias/MSE, sufficiency and factorization, method of moments "
        "and likelihood, Fisher information/confidence/bootstrap/delta, and exact "
        "testing/power/p-values/Wald-score-LR. Each of the seven sets contains "
        "eight fully worked nontrivial problems with metadata, staged hints, a "
        "short answer, and a complete solution.\n\n"
        ...
        "C4 itself is supplied as the current offline HTML reader plus compact "
        "resumable source/backend. Overall C140 remains incomplete: CA02–CA04 "
        "and two capstones still remain.\n\n"
...
def compute() -> tuple[dict[str, bytes], bytes]:
    ...
    build, qa, receipt_identities = validate_c4_boundary()
    ...
    source_entries.update(
        exact_files(
            [
                ...
                (
                    "scripts/package_c140_companion_c4_release.py",
                ...
                (
                    "00_control/C4_MASTERY_BATCH_CONTRACT.md",
                    COMPONENT / "00_control" / "C4_MASTERY_BATCH_CONTRACT.md",
                ),
                *[
    ...
    qa_entries = exact_files(
        [
            ...
            *[(Path(relative).name, COMPONENT / relative) for relative in PINNED_RECEIPTS],
            ...
            (
                "C3_RELEASE_PACKAGE_RECEIPT.json",
                BASE_PACKAGE_RECEIPT,
            ),
            (
                "C3_ZENODO_PUBLIC_READBACK.json",
                BASE_PUBLIC_READBACK,
            ),
        ]
    )
    ...
    additions = [
        new_entry(
            OFFLINE_NAME,
            offline_payload,
            role="partial-c4-offline-html-reader",
            lineage="c140-original-companion-c4",
        ),
        new_entry(
            SOURCE_NAME,
            source_payload,
            role="partial-c4-resumable-source-backend",
            lineage="c140-original-companion-c4",
        ),
        new_entry(
            NOTES_NAME,
            notes_payload,
            role="partial-c4-scope-status-provenance",
            lineage="c140-original-companion-c4",
        ),
        new_entry(
            LICENSE_NAME,
            license_payload,
            role="partial-c4-component-rights",
            lineage="c140-original-companion-c4",
        ),
        new_entry(
            QA_NAME,
            qa_payload,
            role="partial-c4-browser-free-static-qa-evidence",
            lineage="c140-original-companion-c4",
        ),
    ]
    for row, payload in zip(
        ...
        if name in outputs:
            raise RuntimeError(f"C4 release filename collides: {name}")
        outputs[name] = payload
        rows.append(row)
    ...
    manifest_row = new_entry(
        MANIFEST_NAME,
        manifest_payload,
        role="c4-cumulative-union-manifest",
        lineage="c140-original-companion-c4-union",
    )
    ...
    checksum_row = new_entry(
        CHECKSUM_NAME,
        checksum_payload,
        role="c4-cumulative-union-checksums",
        lineage="c140-original-companion-c4-union",
    )
    ...
    root_payload = canonical_json(
        {
            ...
            "coverage": {
                "c140_course": "incomplete after coherent original-companion C4 checkpoint",
                "c140_original_companion": (
                    "partial: D001-D013, SIM001-SIM006, MS00-MS12, CA01"
                ),
                "c4_batch": "complete on its admitted MS00-MS06 mastery boundary",
                "penn_state_spine": "complete: landing/index plus Lesson00-Lesson12",
                "random_completeness_donor": "complete: exact one-page donor",
                "remaining": (
                    "CA02-CA04 and two capstones"
                ),
            },
            "execution_claims": {
            ...
            "schema": "o006.c140.companion-c4-full-union-root.v1",
            "self_exclusion": {
                "filename": ROOT_NAME,
    ...
    root_row = new_entry(
        ROOT_NAME,
        root_payload,
        role="c4-cumulative-union-root-receipt",
        lineage="c140-original-companion-c4-union",
    )
    ...
    if len(final_rows) != BASE_FILE_COUNT + 8:
        raise RuntimeError("C4 publication file count differs")
    publication_bytes = sum(int(row["bytes"]) for row in final_rows)
    if publication_bytes > MAX_PUBLICATION_BYTES:
        raise RuntimeError("C4 publication exceeds the 500,000,000-byte cap")
    if tuple(outputs) != tuple(str(row["filename"]) for row in final_rows):
        raise RuntimeError("C4 output order differs from upload inventory")
    privacy_findings = []
    ...
    if privacy_findings:
        raise RuntimeError(f"privacy findings in new C4 outputs: {privacy_findings}")
...
def main() -> None:
    ...
    if args.contract_only:
        ...
        if errors:
            raise RuntimeError("written C4 package differs: " + ", ".join(errors[:40]))
        state = "written"
    else:
        errors = verify_outputs(outputs, receipt)
        if errors:
            raise RuntimeError("C4 package replay differs: " + ", ".join(errors[:40]))
        state = "verified"
