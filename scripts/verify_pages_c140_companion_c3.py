#!/usr/bin/env python3
"""Verify the cumulative C3 Pages collection without any browser process.

``--contract-only`` validates the complete local Pages assembly against the
live cumulative C3 companion reader and receipts.  It performs no network or
credential access.  ``--write`` and ``--check-only`` retain the hardened C1
static-HTTPS anonymous-byte verifier under C3-specific receipt/schema
...
BUILD_RECEIPT = COMPONENT / "build" / "C3_BUILD_RECEIPT.json"
QA_RECEIPT = COMPONENT / "build" / "C3_QA_RECEIPT.json"
RECEIPT = ROOT / "00_control" / "GITHUB_PAGES_RECEIPT_2026-08-29_C140_COMPANION_C3.json"
SCHEMA = "o006.c140.companion-c3.github-pages-readback.v1"
EXPECTED_COLLECTION_FILES = 181
EXPECTED_COLLECTION_BYTES = 22_126_534
EXPECTED_COMPANION_FILES = 57
...
def local_contract() -> dict[str, object]:
    ...
    build = object_file(BUILD_RECEIPT, "C3 build receipt")
    qa = object_file(QA_RECEIPT, "C3 QA receipt")
    ...
    if (
        ...
        raise RuntimeError("live Pages collection differs from the admitted C3 boundary")
    if (
        ...
        or qa.get("schema") != "o006.c140.companion-cumulative-c3-qa.v1"
        ...
        raise RuntimeError("live C3 build/QA boundary differs")
    ...
    if COMPANION_HTML.is_symlink() or not COMPANION_HTML.is_dir():
        raise RuntimeError("live C3 HTML reader is missing or unsafe")
    for path in sorted(COMPANION_HTML.rglob("*")):
        if path.is_symlink():
            raise RuntimeError(f"symlink in live C3 HTML reader: {path}")
        if not path.is_file():
            continue
    ...
    if len(expected) != EXPECTED_COMPANION_FILES or actual != expected:
        raise RuntimeError("Pages companion partition is not byte-identical to the live C3 reader")
    ...
    if (
        ...
        raise RuntimeError("Pages C3 input aggregate differs from the live build receipt")
    return {
...
def configure_engine() -> None:
    ...
    engine.CONTENT_HEADERS = {
        **engine.CONTENT_HEADERS,
        "User-Agent": "O006-C140-companion-c3-static-readback/2026.08.29",
    }
...
def main() -> None:
    ...
    if args.write:
        ...
        existing = object_file(RECEIPT, "C3 Pages receipt")
    ...
    if args.write:
        ...
        if RECEIPT.read_bytes() != payload:
            raise RuntimeError("public C3 Pages receipt deterministic replay mismatch")
        state = "verified"
