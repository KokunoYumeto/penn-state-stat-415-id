#!/usr/bin/env python3
"""Verify the cumulative C2 Pages collection without any browser process.

``--contract-only`` validates the complete local Pages assembly against the
live cumulative C2 companion reader and receipts.  It performs no network or
credential access.  ``--write`` and ``--check-only`` retain the hardened C1
anonymous-byte verifier under C2-specific receipt/schema identities.
"""
...
BUILD_RECEIPT = COMPONENT / "build" / "C2_BUILD_RECEIPT.json"
QA_RECEIPT = COMPONENT / "build" / "C2_QA_RECEIPT.json"
RECEIPT = ROOT / "00_control" / "GITHUB_PAGES_RECEIPT_2026-08-29_C140_COMPANION_C2.json"
SCHEMA = "o006.c140.companion-c2.github-pages-readback.v1"
...
def local_contract() -> dict[str, object]:
    ...
    build = object_file(BUILD_RECEIPT, "C2 build receipt")
    qa = object_file(QA_RECEIPT, "C2 QA receipt")
    ...
    if (
        ...
        or qa.get("schema") != "o006.c140.companion-cumulative-c2-qa.v1"
        ...
        raise RuntimeError("live C2 build/QA boundary differs")
    ...
    if COMPANION_HTML.is_symlink() or not COMPANION_HTML.is_dir():
        raise RuntimeError("live C2 HTML reader is missing or unsafe")
    for path in sorted(COMPANION_HTML.rglob("*")):
        if path.is_symlink():
            raise RuntimeError(f"symlink in live C2 HTML reader: {path}")
        if not path.is_file():
            continue
    ...
    if actual != expected:
        raise RuntimeError("Pages companion partition is not byte-identical to the live C2 reader")
    ...
    if (
        ...
        raise RuntimeError("Pages C2 input aggregate differs from the live build receipt")
    return {
        "browser_processes_used": False,
        "collection_bytes": collection["collection"]["bytes"],
        "collection_files": collection["collection"]["files"],
        "collection_manifest_sha256": collection["collection"]["manifest_sha256"],
...
def configure_engine() -> None:
    ...
    engine.CONTENT_HEADERS = {
        **engine.CONTENT_HEADERS,
        "User-Agent": "O006-C140-companion-c2-static-readback/2026.08.29",
    }
...
def main() -> None:
    ...
    if args.write:
        ...
        existing = object_file(RECEIPT, "C2 Pages receipt")
    ...
    if args.write:
        ...
        if RECEIPT.read_bytes() != payload:
            raise RuntimeError("public C2 Pages receipt deterministic replay mismatch")
        state = "verified"
