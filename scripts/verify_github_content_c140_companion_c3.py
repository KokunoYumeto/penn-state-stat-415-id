#!/usr/bin/env python3
"""Credential-free static-HTTPS readback for the exact C3 content commit.
...
RECEIPT = ROOT / "00_control" / "GITHUB_CONTENT_READBACK_2026-08-29_C3_COMMIT.json"
REPOSITORY = "KokunoYumeto/penn-state-stat-415-id"
PARENT = "269678b1a9fa8421ff3fa5abc218dff114daaaa3"
COMMIT = "1c8f97f02e9bccfdbe4df91dd77af969cd6e33d6"
EXPECTED_FILES = 68
SCHEMA = "o006.c140.c3.github-content-readback.v1"
...
def download(url: str) -> bytes:
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "o006-c140-c3-static-readback/1.0",
        },
        method="GET",
    )
...
def build_receipt() -> dict[str, object]:
    ...
    if resolved_commit != COMMIT or resolved_parent != PARENT:
        raise RuntimeError("local commit ancestry differs from the fixed C3 boundary")
    ...
    return {
        ...
        "commit": COMMIT,
        "parent": PARENT,
        "change_scope": "files changed by commit relative to the fixed parent",
...
def main() -> int:
    ...
    if args.write:
        ...
        print(
            f"C3 commit readback written: {receipt['file_count']} files / "
    ...
    try:
        ...
        raise RuntimeError("C3 commit readback receipt is unavailable") from exc
    if actual != expected:
        raise RuntimeError("C3 commit readback receipt is stale or non-deterministic")
    print(
        f"C3 commit readback check passed: {receipt['file_count']} files / "
