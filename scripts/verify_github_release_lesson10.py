#!/usr/bin/env python3
"""Anonymously verify the public 12-of-14 GitHub release and its assets.

The commit is supplied explicitly because a verifier must never guess which
release is authoritative.  No credential, token, or authenticated API is
used; only public GitHub pages, tag refs, and release downloads are read.
"""
...
DEFAULT_TAG = "v2026.08.26.12of14"
PACKAGE_RECEIPT = ROOT / "build" / "THROUGH_LESSON10_PACKAGE_RECEIPT.json"
RECEIPT = ROOT / "00_control" / "GITHUB_RELEASE_RECEIPT_2026-08-26_THROUGH_LESSON10.json"
...
def compute(tag: str, commit: str) -> bytes:
    ...
    if package.get("file_count") != 9 or len(files) != 9:
        raise RuntimeError("local release package is not the exact nine-file 12-of-14 boundary")
    if package.get("coverage", {}).get("complete_count") != 12 or package.get("coverage", {}).get("next_document") != "Lesson11":
        raise RuntimeError("local package coverage is not 12 of 14")
    if package.get("reader_zip", {}).get("reader_files") != 94 or package.get("reader_zip", {}).get("reader_bytes") != 17_020_141:
        raise RuntimeError("local package does not bind the exact 94-file reader")
    ...
    session.headers.update({"Cache-Control": "no-cache", "Pragma": "no-cache", "User-Agent": "O006-STAT415-anonymous-release-readback/12.0"})
    ...
    return canonical_json({
        "schema": "o006.stat415.github-release-through-lesson10.v1",
        "status": "passed",
        "anonymous": True,
        "authentication_material_used": False,
        "coverage": {"complete_documents": 12, "corpus_documents": 14, "next_document": "Lesson11"},
        "commit": commit,
        "tag": tag,
