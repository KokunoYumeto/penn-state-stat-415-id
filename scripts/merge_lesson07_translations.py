#!/usr/bin/env python3
"""Merge the three bounded Lesson 07 translation maps deterministically."""
...
TEMPLATE = ROOT / "working" / "lesson07_segments.csv"
TARGET = ROOT / "source" / "id-ID" / "lesson07_translation.csv"
BINDINGS = ROOT / "backend" / "lesson07_translation_bindings.jsonl"
RECEIPT = ROOT / "build" / "LESSON07_TRANSLATION_RECEIPT.json"
GLOSSARY = ROOT / "00_control" / "TERMINOLOGY_GLOSSARY_ID_ID.csv"
TERMINOLOGY_QA = ROOT / "working" / "lesson07_terminology_qa.md"
SOURCE_FINDINGS = ROOT / "working" / "lesson07_source_findings.md"
MATH_AUDIT = ROOT / "working" / "lesson07_math_audit.md"
ASSET_CLOSURE = ROOT / "working" / "lesson07_asset_closure.json"
NORMALIZATION_RECEIPT = ROOT / "build" / "LESSON07_NORMALIZATION_RECEIPT.json"
SCRIPT = ROOT / "scripts" / "merge_lesson07_translations.py"
PARTS = {
    "a": ROOT / "working" / "lesson07_translation_part_a.json",
    "b": ROOT / "working" / "lesson07_translation_part_b.json",
    "c": ROOT / "working" / "lesson07_translation_part_c.json",
}
NOTES = {
    name: ROOT / "working" / f"lesson07_translation_part_{name}_notes.md"
    for name in PARTS
}
FIELDS = (
    "segment_id",
...
SEGMENT_COUNT = 237
GLOSSARY_BYTES = 11_254
GLOSSARY_SHA256 = "34ea1bb9fdf7337aabc13bf1e46f0e8cda346c65ccfae3a1cf7cd48fa712f461"
GLOSSARY_ROWS = 109
GLOSSARY_LAST_TERM_ID = "O006-TERM-0109"
PART_RANGES = {"a": (1, 80), "b": (81, 160), "c": (161, 237)}
GLOSSARY_SCOPE = "exact cumulative glossary through the fifteen Lesson 07 decisions"
RECEIPT_SCHEMA = "o006.stat415.lesson07-translation.v1"
TERMINOLOGY_RULE = "cumulative component glossary through O006-TERM-0109"
REQUIRED_TERMS = (
...
def glossary_identity() -> dict[str, object]:
    ...
    if len(data) < GLOSSARY_BYTES:
        raise RuntimeError("Lesson 07 admitted glossary prefix is truncated")
    admitted = data[:GLOSSARY_BYTES]
    if sha256(admitted) != GLOSSARY_SHA256:
        raise RuntimeError("Lesson 07 admitted glossary prefix differs")
    rows = list(csv.DictReader(io.StringIO(data.decode("utf-8"))))
    if (
        len(rows) < GLOSSARY_ROWS
        or rows[GLOSSARY_ROWS - 1]["term_id"] != GLOSSARY_LAST_TERM_ID
    ):
        raise RuntimeError("Lesson 07 admitted glossary row boundary differs")
    return {
...
def expected_part_keys() -> dict[str, set[str]]:
    return {
        name: {f"{DOCUMENT_ID}-S{i:04d}" for i in range(lo, hi + 1)}
        for name, (lo, hi) in PART_RANGES.items()
    }
...
def load_template() -> tuple[list[dict[str, str]], bytes]:
    ...
    try:
        ...
        raise RuntimeError(f"invalid Lesson 07 segment CSV: {exc}") from exc
    if not rows or tuple(rows[0].keys()) != FIELDS or len(rows) != SEGMENT_COUNT:
        raise RuntimeError("Lesson 07 segment CSV header or row count differs")
    expected_ids = [f"{DOCUMENT_ID}-S{i:04d}" for i in range(1, SEGMENT_COUNT + 1)]
    if [row["segment_id"] for row in rows] != expected_ids:
        raise RuntimeError("Lesson 07 segment IDs are not the admitted sequential boundary")
    if any(
        ...
        raise RuntimeError("Lesson 07 pending template fields differ")
    return rows, data
...
def load_parts() -> tuple[dict[str, str], list[dict[str, object]]]:
    ...
    for name in ("a", "b", "c"):
        ...
        try:
            ...
            raise RuntimeError(f"invalid Lesson 07 translation part {name}: {exc}") from exc
        if (
            not isinstance(values, dict)
            or set(values) != expected[name]
            or list(values) != sorted(expected[name])
        ):
            raise RuntimeError(f"Lesson 07 translation part {name} has the wrong key set")
        if any(not isinstance(value, str) or not value.strip() for value in values.values()):
            raise RuntimeError(f"Lesson 07 translation part {name} has an empty target")
        if set(merged).intersection(values):
            raise RuntimeError("Lesson 07 translation partitions overlap")
...
def compute() -> dict[str, bytes]:
    ...
    if set(translations) != {row["segment_id"] for row in rows}:
        raise RuntimeError("Lesson 07 translation parts do not cover all segments")
    ...
    for ordinal, row in enumerate(rows, start=1):
        ...
        if boundary(target) != (leading, trailing):
            raise RuntimeError(f"Lesson 07 boundary whitespace differs: {row['segment_id']}")
        if not target.strip() or "\ufffd" in target:
            raise RuntimeError(f"invalid Lesson 07 target: {row['segment_id']}")
        if source == target:
    ...
    if missing:
        raise RuntimeError(f"required Lesson 07 terminology absent: {missing}")
    present = [term for term in FORBIDDEN_TERMS if term in joined]
    if present:
        raise RuntimeError(f"visible English/superseded Lesson 07 terminology present: {present}")
    ...
    if args.write:
        ...
        for relative, payload in outputs.items():
            ...
            if not path.is_file() or path.read_bytes() != payload:
                raise RuntimeError(f"Lesson 07 translation output differs: {relative}")
        state = "verified"
